"""Likelihoods and priors in PyTorch."""
...
def make_smart_log_prior(mean, std, weight_decay, temperature):
  """Returns the Gaussian log-density and delta given weight decay."""
  # TODO: make more efficient
  def log_prior(params):
    """Computes the Gaussian prior log-density."""
    params = list(params)
    log_prob = sum([torch.distributions.Normal(m, s).log_prob(p).sum().item() for m, s, p in zip(mean, std, params)])
    return log_prob / temperature
