...
class OilSorbent(DiscreteTestProblem, MultiObjectiveTestProblem):
    ...
    _bounds = [
        (0, 1),  # continuous
        (0, 4.99),  # 5 ordinal values
        (0, 4.99),  # 5 ordinal values
        (0, 4.99),  # 5 ordinal values
        (0, 1),  # continuous
        (0, 3.99),  # 4 ordinal values
        (0, 3.99),  # 4 ordinal values
    ]
    dim = 7
    num_objectives = 3
    ...
    def __init__(
        ...
        for k, v in self._discrete_values.items():
            self.discrete_values[k] = torch.tensor(v, dtype=torch.double)
    ...
    def evaluate_true(self, X: Tensor) -> Tensor:
        ...
        for i, V_i in enumerate(X_split):
            ...
            if name in self.discrete_values:
                X_split[i] = self.discrete_values[name][V_i.view(-1).long().cpu()].view(
                    V_i.shape
                ).to(X)
